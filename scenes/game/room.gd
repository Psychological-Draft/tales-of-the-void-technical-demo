...
const PICKUP_SOUND_PATH := "res://Audio/Audio-Item-Adquired.wav"
@export_file("*.ogg", "*.wav", "*.mp3") var music_path: String
@export var items: Array[Interactable]
@export var minimaplocation = Vector2(0,0)
...
func _ready() -> void:
	...
	UI.get_player_pointer().position = UI.get_mini_room_position_by_id(room_id)
#	print(UI.get_player_pointer().position," : ",UI.get_mini_room_position_by_id(room_id))
	UI.exp_scan_room.connect(scan_room)
	if music_path:
...
func advance_actions() -> void:
	...
	if action is AddItemAction:
		...
		if current_actions.size() > 0:
			push_error("Additional actions cannot be carried out after scene $Control/Map/PlayerPointer!")
		...
		advance_actions()
	elif action is SpawnSomethingAction:
		UI.spawn_something(action.scene_path,action.parent)
		advance_actions()
...
func scan_room():
	for item in items:
		Audio.play_sound("res://Audio/Audio-scanner-scanning.wav")
		if item.scannable:
			item.visible = true
